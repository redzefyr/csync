...
| 작은 일 하나 | *아무것도* | 전부가 파이프라인일 필요는 없다. `backlog.md` 의 한 줄짜리 항목은 그냥 처리하면 되고, 결과는 Claude 가 갈 곳에 배분한다 |
| 그걸 제대로 된 작업으로 | *승격 제안을 승인* | 백로그 항목이 순서·세션을 넘겨 살아남을 다음 걸음·findings 를 받을 자리를 필요로 하게 되면 Claude 가 그 사실을 말하고 승격할지 묻는다. 승인하면 계획이 되고 백로그 줄은 사라진다. 혼자 승격하지는 않는다 — 새 계획은 모든 `list` 에 뜨는 행이다 |
| 파이프라인 종료 | *끝났다고 말한다* | Claude 가 **코드에서** 완료를 확인하고, 아무도 시작하지 않은 후속을 먼저 빼내고, 내용을 `notes/` 와 `docs/` 로 배분한 뒤, 계획 파일을 지우고 닫힌 파이프라인 기록에 한 줄을 남긴다 |
...
`setup` 이 제안하는 영문 원본은
[`templates/repo/global-rules.md`](templates/repo/global-rules.md) 이고, 아래는 그
번역이다. 작업 공간 디렉토리 이름을 바꿨다면 `.csync` 를 그에 맞게 바꾼다:

```markdown
## Claude 작업 산출물 (.csync)

- **프로젝트 루트에 `.csync/` 디렉토리가 있으면, 본격적인 작업 전에 `.csync/notes/`의 모든 파일을 Read 도구로 읽는다.** 밟기 쉬운 함정과 뒤집으면 안 되는 결정은 무엇을 하든 알고 시작해야 한다.
- **사용자가 이 세션에서 `/csync`를 직접 실행하기 전까지, `.csync`에서 읽는 것은 `notes/` 뿐이다.** 나머지는 사용자가 그 작업 공간을 열겠다고 말한 뒤의 것이다. 게이트가 닫혀 있는 동안:
  - ⚠️ **`notes/` 바깥으로 나가지 않는다** — 본문의 `[[slug]]` 참조를 따라가지 않고, `plans/`·`docs/`·`backlog.md`·`GRAPH.md`를 grep·ls·검색을 포함한 **어떤 경로로도** 열지 않는다. 슬러그를 해석해야 하면 사용자에게 `/csync open <slug>`를 제안한다.
  - ⚠️ **`.csync/CLAUDE.md`는 노트를 읽을 때 저절로 들어오며, 게이트가 그보다 우선한다.** 예전 사본은 세션 시작 시 `GRAPH.md` 등을 읽으라고 할 수 있지만, 게이트가 닫혀 있는 동안에는 읽지 않는다.
  - ⚠️ **`.csync`에 쓰지 않는다 — `notes/`도 읽기 전용이다.** 게이지·입력 기준·접기·배치 규칙이 전부 지금 읽지 않는 문서들에 있어서, 그것들 없이 고치면 조용히 망가뜨린다. 남길 것이 생기면 쓰지 말고 **사용자에게 `/csync`를 제안한다.**
...
- **문서 구조 규칙의 정본은 csync 스킬의 `references/workspace.md`이고, 여기에는 사본을 두지 않는다.** 사본은 정본이 개정되는 날 뒤처진다. 편의로 붙여넣지 않는다.
- sync 실행 타이밍은 Claude가 직접 판단한다: 메모리나 `.csync` 노트를 저장한 뒤, 작업을 마무리할 때 `/csync sync`(또는 `~/.claude/skills/csync/scripts/csync-sync.sh`)를 실행한다. 결과는 **한 줄**("pull·push 완료")로 보고하고, 히스토리 분기(divergence)나 재시도 후에도 실패하는 push처럼 사용자를 필요로 하는 것은 즉시 보고한다. **스킬이 로드되지 않은 채 스크립트만 돌린 세션에도 그대로 적용된다.**
```
...
첫 두 불릿 — `notes/` 읽기와 읽기 게이트 — 는 다른 걸 다 빼더라도 남길 것이다. 값은 정말로
...
| `<project>/.csync/CLAUDE.md` | 그 프로젝트 | **Claude** — 이 작업 공간에 대한 사실 | **예**, `prj/<name>` 브랜치에 |
...
**`<project>/.csync/CLAUDE.md` — 작업 공간 자신의 사실.** 프로젝트 한 줄 설명, 규칙과
대장(ledger)의 위치, 그리고 이 프로젝트에 고유한 사항(예: 어떤 language server 가
설치되어 있는지). `/csync init` 이 템플릿에서 만들며, **작업 공간 규칙의 사본은 일부러
두지 않는다**: `init` 이후 아무것도 이 파일을 갱신하지 않으므로, 사본은 다음 스킬
업데이트에서 바로 낡는다.

세션 시작 시 로드되지는 않지만, **`.csync/` 아래 파일을 읽는 순간 Claude Code 가 함께
로드한다** — 위의 글로벌 규칙이 모든 세션에서 `notes/` 를 읽게 하므로, 사실상 모든
세션이 읽는다. 그러니 짧게 유지한다.
...
```
.csync/
├── CLAUDE.md          이 프로젝트에 대한 사실, 첫 노트와 함께 로드된다
├── backlog.md         아직 계획이 없는 일, 날짜가 붙은 한 줄씩
├── plans/             파이프라인 하나 = 파일 하나, YYYYMMDD-YYYYMMDD-<slug>.md
├── notes/
│   ├── decisions.md   사용자가 정한 규칙과 그 이유
│   └── knowledge.md   Claude 가 정한 선택, 그리고 밟기 쉬운 것
└── docs/
...
archive 는 영구적이되 개정되지 않는다. 노트 파일과 백로그는 헤더에 각자 **게이지**를
가진다 — 넘으면 Claude 가 알리거나(사용자의 것인 `decisions.md`), 먼저 접고 그래도
모자라면 확장을 요청하거나(Claude 의 것인 `knowledge.md`), 승격·삭제할 것을
제안한다(`backlog.md`). 섞어 두면 무엇을 판단하든 전부 읽어야 하고,
그 상태가 오래되면 아무도 아무것도 읽지 않게 된다. 수명이 경로에 적혀 있으므로
외우지 않아도 된다.

**인덱스 파일은 없다.** `scripts/csync-ledger.sh` 가 계획·백로그·문서를 읽어 매번
목록을 출력한다 — 살아 있는 파이프라인과 다음 걸음, 백로그, 각 문서가 무엇인지,
슬러그가 어디로 해석되는지. 손으로 최신 상태를 유지할 것이 없고, 세션 시작에 읽는
것은 `notes/` 뿐이다.
...
업데이트가 작업 공간 형식 자체를 바꾸면, 기존 작업 공간은 **옛 형태**로 남고 대장이
`OLD SHAPE` 줄로 알린다. 그동안 Claude 는 그 작업 공간의 노트와 백로그에 아무것도 쓰지
않고, `/csync cleanup` 을 기다린다. 순서는 모든 머신 업데이트 → cleanup 이 글로벌
`CLAUDE.md` 의 csync 절을 확인 → 이전(migration)이다. cleanup 은 각 단계를 묻고, 절반만
이전하느니 멈춘다.
