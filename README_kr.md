...
### 3. 스킬을 로드하지 않는 세션을 위해 설치되는 규칙

대부분의 세션은 스킬을 트리거하지 않는다 — `~/.claude/CLAUDE.md` 는 모든 세션 시작에
읽히지만, 스킬은 무언가가 부를 때만 읽힌다. 그래서 몇몇 규칙은 다른 길로 Claude 에게
닿아야 하고, `install.sh` 가 그것을 `~/.claude/rules/` 에 둔다. Claude Code 는 이
디렉토리를 모든 프로젝트의 모든 세션에 로드한다:

- **`csync.md`** — 스킬 자신의 clone 에 있는 [`rules/csync.md`](rules/csync.md) 로의
  링크. 내용은 거기서 읽으면 된다 — 짧다. 사본이 아니라 링크라서 스킬을 업데이트하면
  함께 바뀌고, 각 머신은 자기가 가진 스킬의 규칙으로 동작한다
- **`csync-workspace.md`** — 작업 공간 디렉토리 이름을 담아 생성되는 파일. 위 링크가
  끊긴 것도 알아챈다. Claude Code 는 끊긴 링크를 아무 말 없이 건너뛰는데, 이 파일이
  Claude 에게 설치 스크립트를 다시 돌릴 때까지 작업 공간을 건드리지 말라고 알린다

글로벌 `CLAUDE.md` 에는 아무것도 추가하지 않으며, 추가해서도 안 된다. 거기 붙인 사본은
붙인 날부터 바뀌지 않는다.
...
읽기 게이트의 값은 정말로 파이프라인을 열고 싶을 때 `/csync` 한 번 더 치는 것이고,
사는 것은 무관한 버그 하나로 연 세션이 남의 계획 수백 줄을 컨텍스트로 끌어오지 않는다는
점이다.

#### 글로벌 `CLAUDE.md` 를 쓰던 버전에서 올라올 때

이전 버전은 이 규칙을 `~/.claude/CLAUDE.md` 에 덧붙이게 했다. **모든** 머신에서 스킬을
업데이트하고 `install.sh` 를 다시 실행한 다음에 그 절을 지운다 — 이 파일은 공유되므로,
먼저 지우면 아직 재설치하지 않은 머신에는 규칙이 하나도 남지 않는다. Claude 는
`/csync setup`·`init`·`update`·`cleanup` 중에 제거를 제안한다.

⚠️ 이 변경을 받아오는 `/csync update` 를 실행한 세션은 시작할 때 로드한 업데이트 절차 —
옛 절차 — 를 그대로 따르므로, 이제는 없는 템플릿을 근거로 그 절을 되살리거나 고치자고
제안할 수 있다. 거절하면 된다. 다음 세션부터 새 절차로 동작한다.
...
├── rules/csync.md -> ~/.claude/csync-tool/rules/csync.md   csync 의 규칙, 링크
├── rules/csync-workspace.md                    생성 파일: 작업 공간 이름
...
csync 자신의 규칙은 여기에 넣지 **않는다** — 그 옆, `~/.claude/rules/` 에 설치된다.
...
로드한다** — csync 의 규칙이 모든 세션에서 `notes/` 를 읽게 하므로, 사실상 모든
세션이 읽는다. 그러니 짧게 유지한다.
...
파이프라인을 열지 않은 세션에서 사용자가 `/csync` 나 `/csync sync` 를 직접 입력했을
때만** 그렇다. 세션 초입이나 무관한 작업을 마친 뒤 직접 돌린 sync 는 무엇이
기다리는지 알려주며 끝나고, 이미 한 파이프라인에 들어가 있는 세션은 건드리지 않는다는
뜻이다. Claude 가 스스로 돌리는 sync 는 언제나 한 줄이다 — 표는 plan 을 나열하는데,
plan 은 사용자가 작업 공간을 열기 전까지 닫혀 있기 때문이다. 각 행이 plan 하나다 — 상태 줄과, 다른
...
않고, `/csync cleanup` 을 기다린다. 순서는 모든 머신 업데이트 → 이전(migration)이다. cleanup 은 이를 묻고, 글로벌
`CLAUDE.md` 에 csync 규칙의 옛 사본이 남아 있지 않은지 확인하며, 절반만 이전하느니
멈춘다.
...
않는 링크는 그대로 제거한다: 포인터·프로젝트 목록·hook·csync 의 규칙·`bin/` 래퍼·스킬
링크. **sync
...
**Claude 가 csync 규칙 링크가 끊겼다며 작업 공간을 건드리지 않는다.**
`~/.claude/rules/csync.md` 가 더 이상 스킬에 닿지 않는 것이다 — 대개 clone 을 옮긴 경우다.
설치 스크립트를 다시 실행한다. Cowork 세션에서는 정상이다: Cowork 는 링크된 rules 파일을
건너뛴다.
